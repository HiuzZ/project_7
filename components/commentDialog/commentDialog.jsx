 ...
 * so that UserPhotos component will re-fetch data, and update UI.
 * @param this.props.handleSumbitChange: handle update notification.
 * @param this.props.photo_id: photo id that is being commented.
 */
export default class CommentDialog extends React.Component {
  ...
  handleCommentChange = (e) => this.setState({ comment: e.target.value }); // reflect comment whenever user changes it

  // Handle changes after Comment submitted
  handleCommentSubmit = () => {
    const commentText = this.state.comment; // store the comment text
    this.setState({ comment: "" }); // clear output before UI disappears
    this.setState({ open: false }); // close the comment dialog UI panel

    // Send POST request with comment text in JSON format
    axios
      .post(`/commentsOfPhoto/${this.props.photo_id}`, { comment: commentText }) // send new comment to server
      .then(() => this.props.onCommentSubmit()) // notify upper level component to re-render UI
  ...
  render() {
    return (
      <div className="comment-dialog">
        <IconButton onClick={this.handleClickOpen} style={{ color: "rgba(0, 0, 0, 0.54)" }}>
          <CommentIcon />
        </IconButton>
        ...
        <Dialog open={this.state.open} onClose={this.handleClickClose}>
          ...
          <DialogActions>
            ...
            <Button onClick={this.handleCommentSubmit} style={{ backgroundColor: "#f9bc60", border: "1px solid black" }}>
              Submit
            </Button>
          </DialogActions>
        </Dialog>
